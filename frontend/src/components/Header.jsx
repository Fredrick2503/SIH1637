...
function Header() {
  ...
  return (
    <header className="w-full h-[80px] bg-white flex flex-row justify-ends items-center p-5 shadow-[0_0_10px_rgba(0,0,0,0.5)] fixed  top-0 z-10 ">
        ...
        <div className='w-[40%] flex flex-row justify-end '  ><button type="button" onClick={()=>setlogout()} className='flex flex-row gap-2 cursor-pointer ' > <p className='text-xs'>Log Out</p> <img src={logout} alt="" srcset="" width={"15px"}  className='object-contain ' /> </button></div>
      </header>
  )
}
