  ...
  return (
    <div className="w-screen h-screen flex flex-col items-center relative overflow-y-auto ">
      ...
      <div className=" h-80% w-full my-[95px] ">
        ...
        <div className="w-full px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          {bids && bids.length > 0 ? (
            bids.map((bid) => (
              <Link key={bid.id} to={`/dashboard/bids/${String(bid.id).replace('-', "")}`}>
                <Card
                  label={bid.produce}
                  info_fields={[
                    { key: "Quantity", value: `${bid.quantity} ${bid.metrics}` },
            ...
            <div className="col-span-full text-center py-20 text-gray-500 flex flex-col items-center">
              <p className="text-xl">No bids found.</p>
            </div>
          )}
        </div>
      </div>
